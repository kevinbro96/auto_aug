...
alpha = 2.0
CNN_embed_dim = 8 # latent dim extracted by 2D CNN
epochs = 300
learning_rate = 1.e-3
...
# Model
print('\n[Phase 2] : Model setup')
model = CVAE_s1(d=CNN_embed_dim, num_channels=3)
...
def train(epoch):
    ...
    for batch_idx, (x, y) in enumerate(trainloader):
        ...
        out, out1, out2, _, xi, xd, mu, logvar = model(x)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(testloader):
            ...
            out, out_i, out_x_xi, hi, xi, xd, mu, logvar = model(x)
